					...
					<div class="record-box">
						<div class="table-wrapper">
							...
							<div class="add-button-wrapper">
								...
								>
									<el-icon style="margin-right: 1vh"
										><circle-plus-filled
									/></el-icon>
									Add
								</el-button>
...
const applyFilter = () => {
	...
	if (dateRange.value !== '' && petSelected.value === '') {
		...
		let endDate = new Date(dateRange.value[1].toISOString().split('T')[0]);
		...
		for (let record of recordList.value) {
			...
			if (
				...
				var index = displayedRecordList.value.indexOf(record);
				if (index !== -1) {
...
const createFilter = queryString => {
	return vac => {
		return vac.value.toLowerCase().indexOf(queryString.toLowerCase()) === 0;
	};
};
const loadAll = () => {
	return [
